...
class Drone():
    ...
    prop_pitch = 4.5
    prop_dia = 10.0
    thrust_fac = 4.392e-8 * (pow(prop_dia,3.5)/math.sqrt(prop_pitch)) * 4.23e-4 * prop_pitch
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        if isinstance(world.player, carla.Vehicle):
            ...
            self.d = Drone()
            self.d, wait_time, max_error_coord, max_error_yaw,ret,trav_path,wp_path,ori = load_params(self.d,args.params)
            ret = (ret=="True")
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                if (self.vehicle_mode == CAR_MODE):
                    ...
                    print("Change Drone Mode!")
                    # self.d.controller_update(waypoints[curr][0],waypoints[curr][1],waypoints[curr][2],waypoints[curr][3])
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text = [
            ...
            '',
            'Speed:   % 15.0f km/h' % (3.6 * math.math.sqrt(v.x**2 + v.y**2 + v.z**2)),
