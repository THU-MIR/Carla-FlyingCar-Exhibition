...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        if isinstance(world.player, carla.Vehicle):
            ...
            parser.add_argument('--waypoints', default='/home/tk/carla/Dist/CARLA_Shipping_0.9.14-4-gf14acb257/LinuxNoEditor/PythonAPI/examples/waypoints_sample.txt', type=str,
                            help='Takes in the required waypoint file')
            parser.add_argument('--params', default='/home/tk/carla/Dist/CARLA_Shipping_0.9.14-4-gf14acb257/LinuxNoEditor/PythonAPI/examples/params_sample.txt', type=str,
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                self.d.state[6] = body_transform.rotation.roll *np.pi/180
                self.d.state[7] = body_transform.rotation.pitch *np.pi/180
                ...
                if (self.vehicle_mode == CAR_MODE):
                    ...
                    world.player.set_simulate_physics(False) # 关闭物理碰撞
                    # 起飞离地
                    if self.first_init:
                    ...
                    # print(body_transform.rotation)

                    # if reached(self.current_waypoint, self.d.state[0],self.d.state[1],self.d.state[2],self.d.state[8],self.max_error_coord, self.max_error_yaw):
                    # # print(self.current_waypoint)
                    #     self.current_waypoint[0] = body_transform[0]
                    #     self.current_waypoint[1] = body_transform[1]
                    #     self.current_waypoint[2] = body_transform[2]
                    #     self.current_waypoint[3] = body_transform[3]

                    self.d.controller_update(*self.current_waypoint)
                    l = self.d.dynamics_euler_angles()
                    ...
                    ## 可视化PID效果
                    update_plot(self.time_index*0.1, self.d.theta*180/np.pi)
                    self.time_index +=1
                    # time.sleep(0.01)
                    # print(self.d.theta*180/np.pi, self.d.phi*180/np.pi, self.d.psi*180/np.pi)
                    # print(dir_vec[0]*180/np.pi, dir_vec[1]*180/np.pi, dir_vec[2]*180/np.pi)
                    body_transform.location.x = point[0]
                    body_transform.location.y = point[1]
                    body_transform.location.z = point[2]
                    body_transform.rotation.roll =self.d.theta*180/np.pi
                    body_transform.rotation.pitch = self.d.phi*180/np.pi
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if len(vehicles) > 1:
            ...
            distance = lambda l: math.math.sqrt((l.x - t.location.x)**2 + (l.y - t.location.y)**2 + (l.z - t.location.z)**2)
